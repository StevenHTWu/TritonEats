...
class DelivererJobHistoryScreen extends Component {
  ...
  async componentDidMount() {
    ...
    const response = await trackerApi.get(
        "/auth/delivererHistory", { headers: { Authorization: AuthStr } }
    );
    ...
    return (
        <SafeAreaView forceInset={{top: "always"}}>
          ...
          <View style={styles.main}>
            ...
            {this.state.isEmpty ? (<Text style={styles.emptyMessage}>You haven't completed any deliveries! Once you complete one, a record will show up here.</Text>) :
                (<FlatList style={styles.main} data={this.state.JobHistory}
                      renderItem={({item}) => <DelivererHistoricalOrder orderId={item.order_id}
